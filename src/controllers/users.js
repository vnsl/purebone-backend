...
const cadastrarUsuario = async (req, res) => {
    ...
    try {
        ...
        const quantidadeUsuarios = await knex('users').where({ email }).first();
        ...
        const usuario = await knex('users').insert({'name' : nome, email, 'password' : senhaCriptografada}).returning('*');
